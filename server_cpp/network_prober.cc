...
bool NetworkProber::recvPing(int timeout, int seq){
  ...
  while (std::chrono::steady_clock::now() - start < std::chrono::microseconds(timeout * 1000)){
    ...
    if (select(sock_ + 1, &read_set, nullptr, nullptr, &select_timeout) <= 0){
      continue;
    }
    ...
    if (!recv_hdr || recv_hdr->type != ICMP_ECHOREPLY){
      continue;
    }
    if (ntohs(recv_hdr->un.echo.id) != pid_ ||
        ntohs(recv_hdr->un.echo.sequence) != seq){
      continue;
    }
...
bool NetworkProber::probePort(const std::string &host, int port, int timeout){
  ...
  bool open = (0 == connect(sock, (sockaddr *)&addr, sizeof(addr)));
  close(sock);
  return open;
}